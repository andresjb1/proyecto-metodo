...
function createEstudiante($Carnet, $nombre, $apellido)
{
...
function readEstudiante($Carnet, $nombre, $apellido)
{
    /*INCLUDE PHP AccesoAula.php
...
function readEstudiantexCarnet($carnet)
{
    /*INCLUDE PHP AccesoAula.php
    Referencia a una funcion readAula de AccesoAula.php, en la cual reciba por parametros
    numero, edificio y capacidad
...
function updatEstudiante($Carnet, $nombre, $apellido)
{
    /*INCLUDE PHP AccesoAula.php
...
function deleteEstudiante($Carnet, $nombre, $apellido)
{
    /*INCLUDE PHP AccesoAula.php
...
if (empty($_POST['carnet']) and $_POST['opcion'] != 2 and $_POST['opcion'] != 5) {
    echo "Por favor ingresar el carnet".":".$_POST['carnet'];
}
...
else
{
    ...
    if($op == 1)
    {
        ...
        {
            createEstudiante($id, $name, $lastname);
        }
        else
        {
    ...
    elseif ($op == 2 )
    {
        readEstudiante($id, $name, $lastname);
    }
    elseif ($op == 3)
    {
        if(is_numeric($id) == true)
        {
            updatEstudiante($id, $name, $lastname);
        }
        else
        {
            echo "Solamente numeros en el carnet";
    ...
    elseif ($op == 4)
    {
        deleteEstudiante($id, $name, $lastname);
    }
    elseif ($op == 5)
    {
